...
(function () {
  ...
  // -------- Netlify Blobs sync endpoint --------
  const TOPBAR_STORE_URL = '/.netlify/functions/store';
...
.topbar-pill-dot {
  ...
  box-shadow: 0 0 6px rgba(5, 150, 105, 0.55);
}
.topbar-pill.warn .topbar-pill-dot {
  background: #d97706;
...
.topbar-water-pill:hover { background: rgba(14, 165, 233, 0.14); }
...
.topbar-water-add:active { transform: scale(0.93); }
...
@media (max-width: 480px) {
  ...
  .topbar-pill, .topbar-water-pill { padding: 8px 10px; gap: 5px; }
  .topbar-pill-label { font-size: 9px; letter-spacing: 0.10em; }
  .topbar-pill-count { font-size: 12px; }
  .topbar-water-add { width: 36px; font-size: 17px; }
}
...
<header class="topbar" id="topbar" role="navigation" aria-label="Quick stats">
  ...
  <a href="gym.html" class="topbar-pill" id="topbarStack">
    <span class="topbar-pill-dot"></span>
...
`;
  ...
  async function pushWaterToNetlify(localWater) {
    // Skip push from po-water.html — it has its own full sync loop
    const p = window.location.pathname;
    if (p.endsWith('/po-water.html') || p.endsWith('po-water.html')) return;
    try {
      await fetch(TOPBAR_STORE_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ po_water_v1: localWater })
      });
    } catch {}
  }
